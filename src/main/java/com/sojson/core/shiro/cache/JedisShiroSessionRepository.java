...
public class JedisShiroSessionRepository implements ShiroSessionRepository {
	...
	public void saveSession(Session session) {
		...
		try {
			...
			/**
			 * 这里是我犯下的一个严重问题，但是也不会是致命， 我计算了下，之前上面不小心给我加了0，也就是 18000 / 3600 = 5
			 * 个小时 另外，session设置的是30分钟的话，并且加了一个(5 * 60)，一起算下来，session的失效时间是 5:35
			 * 的概念才会失效
			 ...
			 * long sessionTimeOut = session.getTimeout() / 1000; Long
			 * expireTime = sessionTimeOut + SESSION_VAL_TIME_SPAN + (5 * 60);
			 */

			/*
			 * 直接使用 (int) (session.getTimeout() / 1000) 的话，session失效和redis的TTL
			 * 同时生效
			 */
